            ...
            {!loadingWin && winners !== null && (
              winners.length === 0 ? (
                ...
                winners
                  ...
                  .map((w) => (
                    <WinnerCard key={w.matchId} winner={w} />
                  ))
              )
            )}
          </div>
...
function WinnerCard({ winner }: { winner: Winner }) {
  const isHome = winner.scoreCol !== null
    ...
    <div className="polla-card">
      ...
      <div style={{ marginTop: 10 }}>
        {winner.winners.length === 0 ? (
          <div className="polla-empty" style={{ padding: "14px 10px" }}>
            Nadie acertó el marcador exacto.
          </div>
        ) : (
          winner.winners.map((name, idx) => (
            <div key={idx} className="polla-winner-row">
              <span className="polla-winner-name">{name}</span>
